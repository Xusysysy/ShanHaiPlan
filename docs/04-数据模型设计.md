...
| 数据 | 权威存储 | 副本/派生 | 说明 |
|---|---|---|---|
| 商品目录/图片 | F2 `catalog.db` | 客户端缓存 | F2 是商品唯一写入口 |
| 实时库存 | F1 内存表 + 快照文件 | F2 只读副本（WS 推送）；BE 期末对账 | **唯一可写点在 F1** |
| 用户登记（wxid+指纹） | F1 `registry.db` | BE 导出汇总 | — |
| Outbox 订单队列 | F1 `outbox/*.wal`（JSONL） | — | 投递 ACK 后归档 |
| 订单账本 | BE `core.db.orders` | F1 缓存（短期） | 全场唯一事实 |
...
- 版本号：每次推送增量携带单调递增 `inv_version`，F2 丢弃乱序旧版本。
...
CREATE TABLE stock_replica (               -- F1 推送的只读副本
    sku        TEXT PRIMARY KEY REFERENCES products(sku),
    available  INTEGER NOT NULL,
    inv_version INTEGER NOT NULL,            -- 乱序保护
    updated_at INTEGER NOT NULL
);
```
...
| 查询 | 位置 | 方案 |
|---|---|---|
| 库存读（匿名 500 QPS） | F2 | 内存 LRU + `stock_replica` 命中，无磁盘读 |
...
| 数据 | 活动 4h 内 | 活动后 |
|---|---|---|
| F1 WAL/快照 | 持续写 | 归档至运维笔记本 + U 盘双份，保留 30 天后删除 |
| BE core.db | 持续写；每 30min `VACUUM INTO` 到备份目录 | 活动结束全量导出 CSV + db 快照双介质；30 天后按 NFR-S5 清理 |
| F2 目录 | 持续读 | 保留至下届复用或归档 |
| 审计日志 | journald | 随服务器归档 30 天 |
